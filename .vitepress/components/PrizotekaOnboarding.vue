...
/* Плавная анимация float — только transform, без filter в keyframes */
@keyframes ob-float {
  0%,100% { transform:translateY(0) scale(1) }
  50% { transform:translateY(-14px) scale(1.06) }
}

/* Статический glow через filter на классе (не анимируется = не мерцает) */
...
/* Shark eyes — тоже статический filter + простой float */
.ob__shark {
  margin-bottom:28px;
...
.ob {
  min-height:calc(100vh - 64px); /* оставляем место под навбар VitePress */
...
@media (max-width: 768px) {
  .ob { padding:8px 16px 16px; min-height:calc(100vh - 64px) }
  .ob__slide { min-height:240px }
  .ob__icon { margin-bottom:12px }
  .ob__icon svg { width:80px; height:80px }
  .ob__shark { margin-bottom:10px }
  .ob__shark svg { width:88px; height:56px }
  .ob__title { font-size:26px; line-height:30px; margin-bottom:12px }
  .ob__desc { font-size:14px }
  .ob__park-title { margin-bottom:20px; font-size:28px }
