      ...
      <div class="cabinet-row">
        ...
        <div class="phone-mockup">
          ...
          <div style="padding: 6px 16px; display: flex; align-items: center; gap: 8px; position: relative; z-index: 1">
            ...
            <span style="font-family: var(--font-mono); font-size: 10px; font-weight: 700; color: var(--lime); letter-spacing: 0.05em">LK.B00M.FUN</span>
          </div>
          ...
          <!-- No shark-eyes watermark — just flex spacer + bottom bar -->
          <div style="flex: 1" />
...
@media (max-width: 768px) {
  .cabinet-row { flex-direction: column; align-items: center; gap: 32px; }
  .cabinet-text { text-align: left; }
  .cabinet-btn-wrap { display: flex; justify-content: center; }
}

.phone-mockup { width: 256px; height: 480px; border-radius: 20px; background: linear-gradient(165deg, #1a1840, var(--bg-night)); border: 2px solid rgba(255,0,128,0.2); animation: phoneBreathe 2.5s ease-in-out infinite; position: relative; overflow: hidden; flex-shrink: 0; display: flex; flex-direction: column; }
...
/* Equal stripe width (3px) and gap (3px) */
.progress-stripes { position: absolute; inset: 0; background-image: repeating-linear-gradient(110deg, rgba(255,255,255,0.15) 0px, rgba(255,255,255,0.15) 3px, transparent 3px, transparent 6px); background-size: 12px 100%; animation: progressStripes 0.6s linear infinite; }
