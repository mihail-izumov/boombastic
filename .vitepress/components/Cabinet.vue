        ...
        <div class="cabinet-row">
          ...
          <div class="cabinet-text">
            ...
            <div class="cabinet-btn-wrap">
              <CutBtn label="Войти в кабинет" lime compact :onClick="openLogin" />
            </div>
          </div>
        </div>
      </div>
...
<style scoped>
.cabinet-center { display: flex; justify-content: center; }
.cabinet-row {
  display: flex; gap: 44px; align-items: center;
  max-width: 660px; width: 100%;
}
.cabinet-text { flex: 1; min-width: 220px; }
.cabinet-btn-wrap { margin-top: 8px; }

@media (max-width: 768px) {
  .cabinet-row { flex-direction: column; align-items: center; gap: 28px; max-width: 100%; }
  .cabinet-text { text-align: left; }
  .cabinet-btn-wrap { display: flex; justify-content: center; margin-top: 20px; }
}

.phone-mockup { width: 240px; height: 460px; border-radius: 20px; background: linear-gradient(165deg, #1a1840, var(--bg-night)); border: 2px solid rgba(255,0,128,0.2); animation: phoneBreathe 2.5s ease-in-out infinite; position: relative; overflow: hidden; flex-shrink: 0; display: flex; flex-direction: column; }
...
/* Equal stripes: 4px filled, 4px gap = 8px period, slow 2s animation */
.progress-stripes {
