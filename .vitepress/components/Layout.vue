...
onMounted(() => {
  ...
  function setupLocalNavHiding() {
    ...
    if (!navScreen) {
      ...
      setTimeout(setupLocalNavHiding, 500)
      return
    }
    
    const hideLocalNav = () => {
      const ln = document.querySelector('.VPLocalNav')
      if (ln) {
        ln.style.display = navScreen.classList.contains('open') ? 'none' : ''
      }
    }
    
    // Наблюдаем за изменениями класса
    const navScreenObserver = new MutationObserver(hideLocalNav)
