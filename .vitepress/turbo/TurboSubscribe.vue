...
const PAGE_VERSION = 'v1.8-rules-text'
...
function rulesFor (parkAddr, phone, orgFull) {
  return [
    ...
    { h: 'Кто участвует', p: [
      ...
      'Возрастных ограничений нет. Приз участнику младше 18 лет выдаётся в присутствии родителя или иного законного представителя, а его персональные данные обрабатываются с согласия представителя.',
    ...
    { h: 'Как получить приз', p: [
      ...
      'Забрать приз можно на кассе парка в течение одного месяца со дня объявления итогов. При себе нужны Игровая карта и телефон, номер которого привязан к карте, — по нему на кассе подтверждают, что приз выдаётся тому самому человеку.',
 ...
 *   Владелец решил обойтись без вычитки юристом. Три места, где это может
 *   аукнуться, названы в changelog — их стоит перечитать перед первым
 *   розыгрышем, а не после.
 */
const RULES = {
  ohta: {
    org: 'ИП Завернин Э. И.',
