...
const PAGE_VERSION = 'v1.9-rules-fix'

/* Общая часть правил. Различаются между парками только организатор, адрес и
   телефон — всё остальное одинаково, и держать три копии одного текста значит
   гарантированно получить три разные редакции после первой же правки. */
function rulesFor (parkAddr, phone, orgFull) {
  return [
    { h: 'Организатор', p: [
      orgFull + '. Контакт для обращений: spb@bumbastiki.ru, ' + phone + '.',
      'Место проведения: ' + parkAddr + '.'
    ] },
    { h: 'Сроки', p: [
      'Розыгрыш проводится еженедельно с 12 августа 2026 года по 31 декабря 2026 года включительно. Если организатор не объявит об окончании, срок автоматически продлевается на следующий календарный год на тех же условиях.',
      'Первый период — с 12 по 16 августа 2026 года, итоги подводятся 17 августа. Далее период равен календарной неделе: с понедельника 00:00 по воскресенье 23:59, итоги подводятся в понедельник следующей недели. Время московское.'
    ] },
    { h: 'Приз', p: [
      'Пакет «Турбо S» — 15 турбо-игр на аркадных автоматах без выдачи тикетов, действительных в турбо-часы. Один пакет за период.',
      'Приз не имеет денежного выражения, не обменивается на деньги и не передаётся другому лицу. Замена приза организатором не производится.'
    ] },
    { h: 'Кто участвует', p: [
      'Участником становится гость, который за период розыгрыша пополнил Игровую карту на сумму от 1 500 ₽ и приобрёл турбо-пакет в этом парке. Отдельной заявки не требуется — участие учитывается автоматически.',
      'Игровая карта должна быть привязана к номеру телефона: по нему организатор связывается с победителем и по нему же победителя опознают на кассе. Карта без привязанного номера в розыгрыше не участвует, потому что сообщить о выигрыше её владельцу невозможно.',
      'Возрастных ограничений для участия нет. Персональные данные участника младше 18 лет обрабатываются с согласия его родителя или иного законного представителя — так же, как при получении Игровой карты и регистрации в Личном кабинете.',
      'Подписка на расписание турбо-часов бесплатна и в розыгрыше не участвует: это разные вещи, и оформление подписки не даёт ни одного шанса на приз.'
    ] },
    { h: 'Как определяется победитель', p: [
      'Из списка участников за период случайным образом выбирается один. Выбор проводит представитель организатора, результат фиксируется в тот же день и хранится у организатора весь срок проведения розыгрыша.',
      'Заранее определить или предсказать победителя невозможно. Организатор не влияет на результат выбора.'
    ] },
    { h: 'Как объявляем', p: [
      'Итоги публикуются на странице b00m.fun/turbo и на экранах ТУРБО в парках в день подведения итогов.',
      'Публикуется имя и первая буква фамилии — например, «Мария К.». Участвуя в розыгрыше, гость соглашается на такую публикацию. Если он против, достаточно сказать об этом при получении приза или написать на spb@bumbastiki.ru: тогда вместо имени публикуется только факт, что победитель определён.'
    ] },
    { h: 'Как получить приз', p: [
      'Организатор связывается с победителем по телефону, привязанному к Игровой карте, и по электронной почте, если она указана.',
      'Забрать приз можно на кассе парка в течение одного месяца со дня объявления итогов. При себе нужны Игровая карта и телефон, номер которого привязан к карте, — по нему на кассе подтверждают, что приз выдаётся тому самому человеку. Других проверок не требуется: приз зачисляется на карту победителя.',
      'Если победитель не забрал приз в течение месяца, приз сгорает, а обязательства организатора считаются исполненными. Невостребованный приз в следующий розыгрыш не переносится.'
    ] },
    { h: 'Персональные данные', p: [
      'Организатор обрабатывает данные участников, уже имеющиеся у него в связи с использованием Игровой карты: номер телефона, имя, адрес электронной почты, историю операций по карте. Дополнительно ничего не собирается.',
      'Цели обработки — определение победителя, связь с ним, выдача приза и публикация итогов. Порядок обработки описан в Политике конфиденциальности на b00m.fun/terms.'
    ] },
    { h: 'Прочее', p: [
      'Организатор вправе изменить настоящие правила или прекратить розыгрыш, опубликовав об этом сообщение на b00m.fun/turbo и на экранах в парках. Изменения не распространяются на уже завершившиеся периоды.',
      'Розыгрыш не является лотереей: он не требует внесения платы за участие в самом розыгрыше, а призовой фонд формируется организатором.',
      'Вопросы и обращения — на spb@bumbastiki.ru или по телефону парка.'
    ] }
  ]
}
 ...
 *
 * Почему три: у парков разные организаторы (Охта и Июнь — ИП Завернин Э. И.,
 * Питерленд — ИП Ендовицкий О. С.) и разные адреса. Общий текст на два юрлица
 * был бы неправдой в пункте, который закон требует называть точно.
 *
 * Почему модалкой, а не страницей в /terms (решение владельца 11.08): ссылка
 * наружу уводит гостя с формы, и обратно он не возвращается.
 *
 * ⚠ Текст написан по ответам владельца 11.08 и данным из оферт и политики.
 *   Владелец решил обойтись без вычитки юристом. Открытые места названы в
 *   changelog — их стоит перечитать перед первым розыгрышем, а не после.
 *
 * ⚠ Формат структурный, а не строка HTML. Через `v-html` сюда можно было бы
 *   вставить что угодно, включая скрипт; заголовки и абзацы отдельными полями
 *   такой возможности не дают в принципе.
 *
 * ⚠ Возраст здесь НЕ проверяется на кассе — намеренно. Первая редакция
 *   требовала выдавать приз несовершеннолетнему при законном представителе,
 *   но дата рождения в Личном кабинете необязательна (`policy.md` §2.1.1),
 *   то есть кассир возраста не знает и узнать не может. Правило, которое
 *   нельзя исполнить, хуже отсутствующего: осталась только обработка данных
 *   с согласия представителя — ровно как при получении карты.
 */
const RULES = {
  ohta: {
    org: 'ИП Завернин Э. И.',
    updated: '11.08.2026',
    blocks: rulesFor(
      'г. Санкт-Петербург, Братиславская дорога, д. 3, ТРК «Охта Молл», этаж 3, оф. 3.15',
      '+7 (964) 321-11-00',
      'Индивидуальный предприниматель Завернин Эдуард Игоревич, ИНН 290217217403, ОГРНИП 304290201600071, адрес: 164500, Архангельская обл., г. Северодвинск, ул. Ленина, д. 41, кв. 37'
    )
  },
  piterland: {
    org: 'ИП Ендовицкий О. С.',
    updated: '19.09.2026',
    blocks: rulesFor(
      'г. Санкт-Петербург, Приморский проспект, д. 72, ТРК «Питерлэнд»',
      '+7 (965) 045-75-55',
      'Индивидуальный предприниматель Ендовицкий Олег Сергеевич, ИНН 290201263531, ОГРНИП 307290205900016, адрес: 197374, г. Санкт-Петербург, Приморский проспект, д. 72, литера А, пом. 332'
    )
  },
  iyun: {
    org: 'ИП Завернин Э. И.',
    updated: '11.08.2026',
    blocks: rulesFor(
      'г. Санкт-Петербург, Индустриальный пр., д. 24, лит. А, ТК «Июнь», этаж 3',
      '+7 (964) 321-11-00',
      'Индивидуальный предприниматель Завернин Эдуард Игоревич, ИНН 290217217403, ОГРНИП 304290201600071, адрес: 164500, Архангельская обл., г. Северодвинск, ул. Ленина, д. 41, кв. 37'
    )
  }
...
/* Кнопка видна, если правила написаны хоть для одного парка, — а не только
   когда парк уже выбран. Иначе при прямом заходе гость читает про условие
   «от 1 500 ₽» и не имеет никакой возможности узнать остальное.
   Парк не выбран — кнопка не молчит, а говорит, чего не хватает: тот же
   принцип, что и у кнопки отправки. */
const rulesExist = computed(() => Object.keys(RULES).some(k => RULES[k].blocks.length))
...
function openRules (e) {
  if (!rulesReady.value) {
    // Парк не выбран: правила у парков разные, показать «какие-нибудь» нельзя.
    rulesHint.value = 'Сначала выбери парк — правила у каждого свои'
    return
  }
  rulesHint.value = ''
  ...
  <div class="ts-page" :data-v="PAGE_VERSION">
    <div class="ts-wrap">
      ...
      <section class="ts-note">
        ...
        <!-- Кнопки нет, только пока правила не написаны вовсе: пустая модалка
             хуже отсутствующей ссылки. Если текст есть, кнопка видна всегда —
             и при невыбранном парке подсказывает, а не молчит. -->
        <p v-if="rulesExist" class="ts-note-link">
          <button class="ts-rules-btn" type="button" @click="openRules">Правила розыгрыша</button>
          <span v-if="rulesHint" class="ts-note-hint">{{ rulesHint }}</span>
        </p>
      </section>
...
.ts-note-hint{display:block;margin-top:6px;font-size:13px;color:var(--err)}
