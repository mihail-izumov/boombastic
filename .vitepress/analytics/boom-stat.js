...
const SOURCES = [
  ...
  'tent-june',         // тейбл-тент на кассе Июня
...
/* Последнее открытие страницы: что и когда. Нужно от дублей — см. ниже. */
let lastView = { path: '', t: 0 }

/**
 * Открытие страницы. Зовётся из theme/index.ts на каждый переход.
 *
 * ⚠ ЗАЩИТА ОТ ДВОЙНОГО СЧЁТА. 24.08 в боевой таблице каждое открытие
 *   давало ДВЕ строки с одинаковым временем: первый заход считает и наш
 *   вызов при старте, и хук VitePress onAfterRouteChanged, который на
 *   первой загрузке тоже срабатывает. Убрать один из двух нельзя: хук
 *   срабатывает не во всех версиях, а без ручного вызова потерялось бы
 *   самое важное событие — приход с наклейки. Поэтому считаем оба, но
 *   повтор того же адреса в пределах секунды отбрасываем.
 */
export function pageview () {
  const now = Date.now()
  if (lastView.path === location.pathname && now - lastView.t < 1000) return
  lastView = { path: location.pathname, t: now }
  track('pageview')
}
