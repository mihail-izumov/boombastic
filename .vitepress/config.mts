...
export default defineConfig({
  // appearance: false убирает ВСЕ элементы Appearance из UI
  // Тёмная тема форсируется скриптом ниже (добавляет класс .dark на html)
  appearance: false,
  ...
  head: [
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // СКРИПТ: Принудительная тёмная тема + футер + кнопки навбара
    // ═══════════════════════════════════════════════════════════════════════
    ['script', {}, `
    (function() {
      // 1. ПРИНУДИТЕЛЬНАЯ ТЁМНАЯ ТЕМА
      ...
      function createFooterContent() {
        ...
        html += '<div style="color:var(--vp-c-text-2);margin-top:4px;font-size:14px;">© БумБастик 2025</div>';
        return html;
      }
      ...
      // 3. КНОПКА «ПОДДЕРЖАТЬ» — внутренняя ссылка
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // СТИЛИ: Один в один с Сигналом
    // ═══════════════════════════════════════════════════════════════════════
    ['style', {}, `
    /* --- БРЕНДОВЫЕ ЦВЕТА (как в Сигнале) --- */
    :root {
      --vp-c-brand-1: #347b6c;
      --vp-c-brand-2: #C5F946;
      --vp-c-brand-3: #347b6c;
      --vp-c-brand-soft: rgba(52, 123, 108, 0.14);
      --vp-c-bg-mute: rgba(52, 123, 108, 0.1);
      --vp-c-bg-soft: rgba(52, 123, 108, 0.1);
      --vp-c-divider: rgba(52, 123, 108, 0.2);
      --vp-c-gutter: rgba(52, 123, 108, 0.1);
      --vp-c-text-2: #b3b3b3;
    }

    /* --- СКРЫТЬ ПЕРЕКЛЮЧАТЕЛЬ ТЕМЫ ВЕЗДЕ --- */
    .VPSwitchAppearance { display: none !important; }
    .VPNavScreenAppearance { display: none !important; }
    .VPNavBarExtra .appearance { display: none !important; }
    ...
    .VPMenuItem:hover { background-color: rgba(197, 249, 70, 0.1) !important; }

    /* --- HERO СЕКЦИЯ --- */
    .VPHero .name, .VPHero .text, .VPHero .tagline { color: white !important; }
    .VPHero .tagline a { color: var(--vp-c-brand-2) !important; text-decoration: none; transition: all 0.3s ease; }
    .VPHero .tagline a:hover { color: var(--vp-c-brand-1) !important; text-decoration: underline; }
    .VPHero .VPButton { background-color: var(--vp-c-brand-1) !important; border-color: var(--vp-c-brand-1) !important; color: white !important; transition: all 0.3s ease; text-decoration: none !important; }
    ...
    /* --- ССЫЛКИ В КОНТЕНТЕ --- */
    .VPContent a { color: var(--vp-c-brand-2); text-decoration: none; border-bottom: 1px solid transparent; transition: all 0.3s ease; }
    .VPContent a:hover { color: var(--vp-c-brand-1); border-bottom-color: var(--vp-c-brand-1); }

    /* --- КАРТОЧКИ (FEATURES) — как в Сигнале --- */
    .VPFeature { background-color: #202124 !important; border-color: #3c3c3c !important; }
    .VPFeature .title { color: #C5F946 !important; transition: color 0.25s ease-in-out; }
    .VPFeature .details { color: rgba(235, 235, 245, 0.6) !important; }
    .VPFeature .link-text { color: #347b6c !important; transition: color 0.25s ease-in-out; }
    .VPFeature .link-text .icon { fill: #347b6c !important; }
    a.VPFeature.link:hover { background-color: #2f2f2f !important; border-color: #555 !important; }
    a.VPFeature.link:hover .title,
    a.VPFeature.link:hover .link-text { color: var(--vp-c-brand-2) !important; }

    /* --- SOCIAL LINKS: СКРЫВАЕМ GITHUB ИКОНКИ, ПОКАЗЫВАЕМ ТЕКСТОВЫЕ КНОПКИ --- */
    ...
    /* Кнопка «Поддержать» (лаймовая с анимацией liquid-fluid) */
    ...
    /* --- SVG ИКОНКИ SOCIAL (Telegram и др.) --- */
    ...
    @media (max-width: 768px) {
      ...
      /* Телеграм-иконка в мобильном меню */
    ...
    /* --- УБРАТЬ ВСЕ РАЗДЕЛИТЕЛИ В НАВБАРЕ --- */
    ...
    .footer-row a { color: var(--vp-c-text-2); text-decoration: none; transition: color 0.3s ease; }
    .footer-row a:hover { color: var(--vp-c-brand-2); }
  ...
  themeConfig: {
    ...
    // TODO: заполнить sidebar когда будет контент
    sidebar: {},
    ...
    // ═══════════════════════════════════════════════════════════════════
    // НАВИГАЦИЯ: тестовые пункты (обычный + с выпадающим списком)
    // ═══════════════════════════════════════════════════════════════════
    nav: [
